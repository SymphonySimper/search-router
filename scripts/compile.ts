...
import { withProtocol } from '../src/url.ts';
...
function compileMappings(): Record<string, string> {
	...
	for (const [href, aliases] of Object.entries(MAPPING_CONFIG)) {
		...
		const destination = withProtocol(href);
		validateUrl(destination, 'Mapping destination');
...
function resolveDirectSearch(engine: Extract<SearchEngine, { url: string }>, key: string): { homepage: string; search: string } {
	...
	const homepage = withProtocol(engine.url);
	const search = withProtocol(`${engine.url}${engine.search}`);
	validateUrl(homepage, `Search engine ${key} homepage`);
	validateUrl(search, `Search engine ${key} search URL`);
...
function compileSearchEngines(): { default: string; engines: Record<string, ResolvedSearchEngine> } {
	...
	for (const [key, engine] of Object.entries(engines)) {
		...
		if ('site' in engine) {
			homepage = withProtocol(engine.site);
			search = defaultSearch;
			prefix = `site:${engine.site} `;
			validateUrl(homepage, `Site search engine ${key} homepage`);
		} else {
